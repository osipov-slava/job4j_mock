...
public class UnbindAction implements Action {
    ...
    private static final String URL_UNBIND = "/telegram/unbind";
    ...
    public BotApiMethod<Message> handle(Message message) {
        ...
        var text = "Для отвязки телеграм аккаунта от учетной записи\nВведите email и пароль(через пробел)";
    ...
    public BotApiMethod<Message> callback(Message message) {
        ...
        Object result;
        try {
            result = authCallWebClint.doPost(URL_UNBIND, person).block();
        } catch (Exception e) {
        ...
        if (mapObject.containsKey(ERROR_OBJECT)) {
            text = "Ошибка /unbind: " + mapObject.get(ERROR_OBJECT);
            return new SendMessage(chatId, text);
        }

        text = "Ваш телеграм аккаунт отвязан " + sl
               + "от email: " + email;
        return new SendMessage(chatId, text);
    }
}
